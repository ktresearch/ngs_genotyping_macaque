...
outputfile = f'count_multihit_{str(min_length)}.txt'
subprocess.run([f'mkdir coverage_multihit_{min_length}'], shell=True)
subprocess.run([f'mkdir coverage_multihit_{min_length}_plot'], shell=True)
...
with open(outputfile,'w') as out:
    ...
    for allele in count.keys():
        ...
        plot_x = []
        plot_y = []
        outputfile_depth = f'coverage_multihit_{min_length}/{filename}.txt'
        ...
        plt.ylim(0,)
        plt.savefig(f'coverage_multihit_{min_length}_plot/{filename}.pdf')
        plt.close()
